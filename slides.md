...
- **아이디어 검증**: OpenClaw를 서버리스에 올릴 수 있을까? → ECS/Fargate로 **구현 달성** → 다음 아이디어로
- **비용 검증**: "월 $1 가능할까?" → 3가지 결정으로 **$0.27/월 달성** → 다음 아이디어로
- **기술 검증**: "콜드 스타트 해결 가능할까?" → Lambda 이관으로 **1.35초 달성**, Smart Routing으로 최적 런타임 자동 선택 → 다음 아이디어로
...
| TDD + 촘촘한 검증으로 속도+안정성 | CDK로 인프라도 코드로 관리 |
